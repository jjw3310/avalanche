...
// seasonal minting part
    ...
    // check input data before call this function!!!
     ...
     private
    {
        ...
        if(_season > 0 && bytes(_seasonName).length > 0) {
            require(seasonNameToNum[_seasonName] == _season, "_ERR[0005]:Check _season or _seasonName");
        }
        if(_season > 0) {
            require(_season > 0 && _season <= seasonOpened, "_ERR[0004]:Check your _seanson value");
        } else if(bytes(_seasonName).length > 0 ) {
            require(seasonNameToNum[_seasonName] > 0, "_ERR[0005]:Check _season or _seasonName");
        ...
        for(uint i=0; i < seasonList[seasonNum].owners.length; i++){
            ...
            require(ownCnt <= 10, "_ERR[1007]:Can't mint more than 10 in a season");
        }
